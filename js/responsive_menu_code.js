...
$(document).ready(function() {
	...
	/* Defines the menu_bottom value (where the menu is located).*/
	function menuBottom(){
		...
		if (key_pressed == "9") {
			$(".row_01-content:eq("+current_index+")").each(
				...
				if ($(".row_01-content:eq("+current_index+") a:eq(+"+last_element_of_content+")").is(":focus")) {
					...
					$(".row_01 a:eq("+current_index+")").removeClass("row_01-mouseover_nobackground");
					$(".row_01-content").addClass("hide");
				}
		}
		else{
	...
	"mouseenter focusin", function () {
		if (burger_menu_active===false){
			...
			$(".row_01 a:eq("+current_index+")").addClass("row_01-mouseover_nobackground");
			$(".row_01-content:eq("+current_index+")").each(
				function() {
					last_element_of_content=(($("a", $(this)).length)-1).toString();
				}
			...
			for (var i = 0; i < $(".row_01").length; ++i) {
				if(i!=current_index){
					$(".row_01:eq("+i+")").removeClass("row_01-mouseover");
					$(".row_01:eq("+i+") a").removeClass("row_01-mouseover_nobackground");
	...
	$(".row_01-content").on(
	"mouseenter focusin", function () {
		current_index=$(this).parent().children(".row_01-content").index(this).toString();
		if (burger_menu_active===false){
			$(".row_01-content:eq("+current_index+")").removeClass("hide");
			$(".row_01:eq("+current_index+")").addClass("row_01-mouseover");
			$(".row_01 a:eq("+current_index+")").addClass("row_01-mouseover_nobackground");
	...
	$(".row_01-content a, .row_01 a, .row_01").click(function(e){
		...
		$(".row_01-content").addClass("hide");
		$(".row_01").removeClass("row_01-mouseover");
		$(".row_01 a").removeClass("row_01-mouseover_nobackgrund");
		if (burger_menu_active===true) {
			slideBurgerMenuUp();
			burger_menu_active=true;
		}

		var navbar_text_link = $(this).attr("href");
		if (typeof navbar_text_link==="undefined") {
			navbar_text_link = $(this).find("a").attr("href");
		}
		if((isExternal(navbar_text_link)==true) || (navbar_text_link.includes("/")==true) || (navbar_text_link.includes(".html")==true)){
			window.location.href = navbar_text_link;
		}
